...
class AppStore extends Component {
  state = {activeApp: tabsList[0].tabId, searchInput: ''}

  filtering = () => {
    const {activeApp} = this.state
    const filteredList = appsList.filter(each => each.category === activeApp)
    return filteredList
  }
  ...
  render() {
    const {activeApp} = this.state
    const filteredList = this.filtering()
    const {searchInput} = this.state
    const finalUpdate = filteredList.filter(each =>
        ...
        <ul className="tabsList">
          {tabsList.map(each => (
            <TabItem
              ...
              active={each.tabId === activeApp}
            />
          ))}
        </ul>
        <ul className="appsList">
          {finalUpdate.map(eachItem => (
            <AppItem appListDetails={eachItem} key={eachItem.appId} />
          ))}
        </ul>
      </div>
    )
